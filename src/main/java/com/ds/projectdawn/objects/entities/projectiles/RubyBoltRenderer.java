...
@OnlyIn(Dist.CLIENT)
public class RubyBoltRenderer extends EntityRenderer<RubyBoltEntity>
{
    ...
    public RubyBoltRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    public void render(RubyBoltEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
    ...
    private static void render(IVertexBuilder vertexBuilder, Matrix4f matrix4F, Matrix3f matrix3F, int lightmap, float x, int y, int p_229045_6_, int p_229045_7_) {
        ...
        vertexBuilder.vertex(matrix4F, x - 0.5F, (float)y - 0.25F, 0.0F).color(227, 38, 52, 255).uv((float)p_229045_6_, (float)p_229045_7_).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap).normal(matrix3F, 0.0F, 1.0F, 0.0F).endVertex();
    }

    @Override
    public ResourceLocation getTextureLocation(RubyBoltEntity entity) { return MAGIC_ORB_TEXTURE; }
