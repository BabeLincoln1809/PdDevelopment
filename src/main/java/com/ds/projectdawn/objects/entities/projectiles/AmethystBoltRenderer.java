...
@OnlyIn(Dist.CLIENT)
public class AmethystBoltRenderer extends EntityRenderer<AmethystBoltEntity>
{
    private static final ResourceLocation MAGIC_ORB_TEXTURE = new ResourceLocation("projectdawn:textures/entity/projectile/magic_orb.png");
    private static final RenderType magic_ball_render = RenderType.entityCutoutNoCull(MAGIC_ORB_TEXTURE);

    public AmethystBoltRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    public void render(AmethystBoltEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
    ...
    private static void render(IVertexBuilder vertexBuilder, Matrix4f matrix4F, Matrix3f matrix3F, int lightmap, float x, int y, int p_229045_6_, int p_229045_7_) {
        ...
        vertexBuilder.vertex(matrix4F, x - 0.5F, (float)y - 0.25F, 0.0F).color(141, 106, 204, 255).uv((float)p_229045_6_, (float)p_229045_7_).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap).normal(matrix3F, 0.0F, 1.0F, 0.0F).endVertex();
    }

    @Override
    public ResourceLocation getTextureLocation(AmethystBoltEntity entity) { return MAGIC_ORB_TEXTURE; }
